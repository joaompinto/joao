...
class Agent:
    ...
    def request(self, message, tools=None, auto_use_tools=True):
        """Send a message to the model and get a response
        
        Args:
            message (str): The message to send
            tools (list): Optional list of tools to make available
            auto_use_tools (bool): If True, automatically execute any tool calls
        ...
        try:
            ...
            if is_debug_enabled('sent'):
                self.debug_print("======= Sending message to model", component='sent')
                self.debug_print(f"System prompt:\n{self.system_prompt}", component='sent')
                self.debug_print(f"Message:\n{message}", component='sent')
                if tools:
                    self.debug_print("Tools:", component='sent')
                    for tool in tools:
                        self.debug_print(f"- {tool.__name__}", component='sent')
            ...
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[{"role": "system", "content": self.system_prompt}] + self.messages,
                tools=self.tools_handler.get_tool_schemas() if tools else None,
                n=1,
                temperature=self.temperature,
            )
            ...
            self.messages.append({
                "role": "assistant",
                "content": answer.content or "",
                "tool_calls": answer.tool_calls if has_tool_calls else None
            })
            ...
            if has_tool_calls:
                ...
                self.tools_handler.set_tool_calls(answer.tool_calls)
                if auto_use_tools:
                    # Execute tools and get model's response
                    return self.use_tools(auto_update=True)
            ...
            if is_debug_enabled('agent'):
                self.debug_print(f"Error in request: {e}")
            raise
