...
from typing import Optional, List, Callable, Union, Iterator
...
class Agent:
    def __init__(self, system_prompt: str = None, temperature: float = 0, tenant_prefix: str = None):
    ...
    def request(
        ...
        if stream:
            return self._stream_response(self.client.chat.completions.create(**kwargs))
        ...
        if use_tools and answer.tool_calls:
            return self.use_tools(autoupdate=True)
            
        return answer.content
    
    def _stream_response(self, response_stream) -> Iterator[str]:
    ...
    def use_tools(self, autoupdate=True):
        ...
        try:
            tool_response = self.tools_handler.execute_tool_calls()
